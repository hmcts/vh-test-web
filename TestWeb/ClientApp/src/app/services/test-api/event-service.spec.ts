...
describe('EventService', () => {
    ...
    it('should call the test api to create a partcipant event', async () => {
      testApiService.sendEvent.and.returnValue(Promise.resolve());
      const result = await service.createParticipantEvent(eventModel.conference_id, eventModel.participant_id,
        eventModel.event_type, eventModel.transfer_from, eventModel.transfer_to);
      expect(testApiService.sendEvent).toHaveBeenCalled();
      expect(result).not.toBeNull();
    });
    ...
    it('should throw an error if call to test api to send an event fails', async () => {
      ...
      await expectAsync(service.createHearingEvent(eventModel.conference_id, eventModel.event_type, eventModel.participant_id))
